...
- Frontend (`index.html` + `app.js` + `style.css`) is a drag-and-drop upload UI with a **model-selection dropdown**, a **Photo Upload / Live Camera tab switcher**, an animated "scanning" state, and a diagnosis report (symptoms / treatment / prevention / confidence bar / which model produced the result).
- **Live camera capture:** the "📷 Use Camera" tab uses `navigator.mediaDevices.getUserMedia()` to show a live video feed (defaults to the back/environment camera on phones, with a switch-camera button), and a shutter button captures a frame via `<canvas>` and feeds it into the same analysis pipeline as a file upload — handy for live demos without needing to first save a photo to disk. ⚠️ Browsers only allow camera access on `https://` or `localhost` — it won't work if the Flask app is served over plain `http://` on a non-localhost address.
- **Auto-capture (quality-gated):** while the camera is open, a lightweight JS check runs ~every 150ms on a small downsampled frame and only auto-fires the shutter once the live feed is simultaneously well-lit, in focus, and steady for ~1 second straight — similar in spirit to how Face ID won't scan a blurry/dark frame. It checks three signals purely in-browser (no extra model/library):
  - **Brightness** — average grayscale value, rejects frames that are too dark or blown out
  - **Sharpness** — variance of a Laplacian-style edge filter, rejects blurry/out-of-focus frames
  - **Motion** — frame-to-frame pixel difference, rejects frames while the hand/leaf is still moving
  A colored border around the live feed (red → amber → green) and a status caption ("Too dark", "Focusing…", "Looking good — hold still") give real-time feedback, and it flashes white + auto-captures once the streak requirement is met. The manual shutter button still works as an override/fallback at any time, and there's an on/off checkbox for auto-capture. The thresholds (`QUALITY` object at the top of the auto-capture section in `app.js`) are reasonable starting points — they may need tuning for a specific camera/lighting setup.
